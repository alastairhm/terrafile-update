...
def main():
    ...
    parser.add_argument(
        ...
        "--output",
        type=str,
        help="Input Terrafile YAML, default=Terrafile.new",
        default="Terrafile.new",
    )
    parser.add_argument(
        "-m", "--major", action="store_true", help="Update major versions of modules"
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "-c", "--check", action="store_true", help="Just check no output"
    )
    ...
    if os.path.isfile(args.input):
        ...
        for repo in terrafile:
            ...
            response = http.request("GET", api, headers={"PRIVATE-TOKEN": token})
